...
<%
	...
	int pageNumber;				// 현재 페이지 넘버		23 -> 3page
	...
	// 끝페이지 계산
	if((pageNumber + 1) % pageCountPerScreen == 0){	// pageNumber = 9 19
	...
	System.out.println("totalPageCount:" + totalPageCount);	// 총 페이지 수 : [1] ~ [12] -> 12
...
<div style="float: left; width: 96%; text-align: center;">
	...
	if(screenStartPageIndex > 1){
		...
		<a href="#none" title="이전페이지" onclick="goPage('<%=screenStartPageIndex - 1 %>')">
			<img alt="12312" src="image/arrow_back.gif" style="width: 9px; height: 9px;">
		</a>&nbsp;
		<%
	}
	...
	if(screenEndPageIndex < totalPageCount){
		...
		<a href="#none" title="다음페이지" onclick="goPage('<%=screenEndPageIndex %>')">
			<img alt="123123" src="image/arrow_next.gif" style="width: 9px; height: 9px;">
		</a>
		<%
	}
